...
fracking_clean <- fracking |> 
  ...
  # clean column names ----
  janitor::clean_names() |> # update column headers to snake case
  
  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) |> # remove 'AM' from the 'job_start_date' column
  mutate(datetime_start = mdy_hms(job_start_date)) |> # convert 'job_start_date' from a string to a datetime as a new column 'datetime_start'
  mutate(year = year(datetime_start)) |> # create new column 'year' for year of 'datetime_start'
  ...
  # rename 'state_name' to 'sn' (not necessary) ----
  rename(sn = state_name) |> 
  
  # convert all words in 'sn' to title case ----
  mutate(sn = str_to_title(sn)) |> 
  
  # fix misspelled state names in 'sn' column ----
  mutate(sn = case_when(
    ...
    sn == "Wyominng" ~ "Wyoming", 
    TRUE ~ sn # copy over rest of state names as-is
  )) |> 
  
  # remove rows that contain '?' mark ----
