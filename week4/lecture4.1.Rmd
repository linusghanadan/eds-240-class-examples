...
*Data rankings*: Comparing categorical groups based on their numeric values
...
```{r}
# Plotting two groups side by side
jobs_clean |> 
  filter(year == 2016) |> 
  slice_max(order_by = total_earnings, n = 10) |> 
  pivot_longer(cols = c(total_earnings_female, total_earnings_male), names_to = "group", values_to = "earnings_by_group") |> 
...
## Aside: geom_col() vs geom_bar()

Use *geom_col()* when your data is already summarized or you have a variable in your data set that includes y-axis values, which will map to the height of the bars.

Use *geom_bar()* if you want to ggplot to count up numbers of rows and map those counts to the height of bars in your plot. 
